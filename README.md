...
```
 ...
 - fifoデータの取得とFFTを実行するスレッド
    ...
    - FFT解析には、[chfft](https://crates.io/crates/chfft) と [rustfft](https://crates.io/crates/rustfft) を試しました。chfft の方か CPU 使用率がちょっとだけ少なかったので、chfftを採用しました。（日本の方がつくったよーです。[こちら](https://qiita.com/chalharu/items/9a6b23170349ae013f40) ）
...
# 参考にしたサイトと謝辞
 ...
 - なお、[Rustのターミナル操作crateいろいろ](https://meganehouser.github.io/2019-12-11_rust-terminal-crates.html) という記事と、そちらの方の[rustmatrix](https://github.com/meganehouser/rustmatrix) のソースコードを参考にさせていただきました。
    - プログラムの構成は、まんま rustmatrix の真似です。御礼申し上げます。ありがとうございました。
...
```
    - TeraTerm が ANSIエスケープシーケンスの拡張色変更[こちら](https://qiita.com/PruneMazui/items/8a023347772620025ad6#%E5%87%BA%E5%8A%9B%E8%89%B2%E3%81%AE%E5%A4%89%E6%9B%B4%E6%8B%A1%E5%BC%B5)に対応してないので、以下のコードを挿入して標準色に変更。
```diff
